# ADR-005: Tool Granularity — Four Focused Tools over One Generic Tool
...
**Chosen: Four focused tools**, each mapping to a distinct capability:

| Tool | Capability |
|------|------------|
| `qlik_get_sheet_details` | Read existing dashboard layouts |
| `qlik_get_hypercube_data` | Retrieve governed data slices |
| `qlik_create_sheet` | Build temporary analysis views |
...
4. **Not too many, not too few**: More than ~6 tools increases the cognitive load on the LLM (which tools overlap? which to use?). Fewer than 3 forces overloading. Four is the sweet spot for this domain.

### Trade-offs accepted:

- **No raw API access**: Advanced users cannot make arbitrary Engine API calls. They must use the four predefined tools. This is intentional — it constrains the attack surface.
